...
public extension Fn {
	...
	static func throttle(_ interval: TimeInterval, on queue: DispatchQueue = DispatchQueue.main, function f: @escaping () -> Void) -> () -> Void {
		{ [cancel = SerialDisposable()] in
			cancel.innerDisposable = Control.run(after: interval, on: .global(qos: .userInteractive)) {
...
public extension Fn {
	...
	static func lazy<A>(_ generator: @escaping () -> A) -> () -> A {
		...
		return {
			switch state {
			case .generator(let f): return Control.with(f()) { state = .value($0) }
